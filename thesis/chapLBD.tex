...
There are two main objections to the previous identification strategy:
\begin{itemize}
...
The current alternative for close wins employs a ranking mechanism to label close wins when all the firms involved in the auction were close in ranking. The argument here is that, given a well constructed ranking, winning a contract against closely placed opponents is attributable to close factors.

Obviously, the main issue is how to construct a good ranking measure. We proceed by  modeling each auction as a multi-player game (in the non-economic sense of the term) event in which firms gain points by winning the project and lose points by not winning it. We award and substract points based on a modified ELO algorithm suited for multi-player games.
...
Having assigned to each firm a ranking for each point in time(i.e for each auction), we label a win as a "close win" to the one where the highest rank among the bidders for the auction was not more than 3\% higher than the lowest rank among them. This yields around of close wins in the final sample of analysis classified as a close win. In the robustness checks we analyze both i) different values for the won/lost points after an auction and ii) the threshold in ranking for a close win.

Just as before, in Table \ref{tab:closewins_alt2_desc} we present summary statistics for close wins identified via close wins.

\input{C:/repos/learn-doing/thesis/tables/table_closewins_alt2_desc.txt}



In the analysis, we drop the first two years of data to allow for a period of rank adjustment. This is necessary since the algorithm does not work well when the average rank is not clearly defined.
